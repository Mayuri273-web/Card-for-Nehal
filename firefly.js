...
(function restoreBgmOnGame(){
  ...
  function showResumeCTA() {
    ...
    btn.addEventListener('click', async () => {
      try { await bgm.play(); btn.remove(); } catch(e){}
    });
  }
  ...
  try {
    ...
    if (raw) {
      ...
      if (st.playing === true) {
        bgm.play().then(persist).catch(showResumeCTA);
      } else persist();
    } else {
...
const messages = [
  "Sometimes you test my patience so much… and then one tiny smile from you and I’m acting hopelessly in love again.",
  "I love you Babii",
  "Even after everything we survived… the fights, the fears — you still pull me close like I’m your safest place.",
  "Remember our long rides? You acting cool, me holding you like my whole world depends on it.",
  "No matter how much drama we go through, you still call me morni and suddenly I forget all my attitude."
];
...
class Firefly {
  ...
  draw(){
    ...
    ctx.fillStyle = this.isTarget
      ? `rgba(250,220,140,${0.10 + this.flash*0.06})`
      : `rgba(250,220,140,${0.08 + this.flash*0.04})`;
    ctx.fillRect(this.x-3, this.y-3, this.size+6, this.size+6);

    ctx.fillStyle = this.isTarget
      ? `rgba(224,184,90,${alpha})`
...
function startGame(){
  ...
  fireflies[targetIndex].isTarget = true;
...
function loop(){
  ...
  for(const f of fireflies){ f.move(); f.draw(); }
  animId = requestAnimationFrame(loop);
}
...
function showNote(){
  ...
  dynamic.textContent = messages[Math.floor(Math.random() * messages.length)];
  ...
  if(!popup._backdropListener){
    popup._backdropListener = (ev)=>{
      if(ev.target === popup) closeNote();
    };
    popup.addEventListener('click', popup._backdropListener);
  }
}
